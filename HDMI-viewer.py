...
RIGHT_CAMERA_INDEX = 1    # Right display (default)
    ...
    def __init__(self, camera_index: int, test_mode: bool = False, label: str = "FEED"):
        ...
        if test_mode:
            ...
            if self.cap and self.cap.isOpened():
                ...
                print(f"Camera {camera_index}: {w}x{h} @ {fps:.1f} FPS")
            else:
...
class DualVideoViewer(QWidget):
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'info_overlay'):
            margin = 20
            self.info_overlay.move(margin, self.height() - self.info_overlay.height() - margin)
    ...
    def set_layout_mode(self, mode: LayoutMode):
        ...
        if mode == LayoutMode.DUAL:
            ...
            print(f"Layout: DUAL (Left: cam {self.left_camera_index}, Right: cam {self.right_camera_index})")
